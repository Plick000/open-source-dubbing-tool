...
# ✅ Preferred source location inside dubbing-flask/
...
def ensure_dir_win(dir_win: str) -> None:
    ps(f'New-Item -Force -ItemType Directory -Path "{dir_win}" | Out-Null')


def write_text_win(path_win: str, content: str) -> None:
    # BOM-less UTF-8 (same robustness as your reference runner)
...
def test_path_win(path_win: str) -> bool:
    p = path_win.replace('"', '""')
    out = ps_out(f'Test-Path -LiteralPath "{p}"')
...
def main():
    ...
    if src_titles_primary.exists():
        ...
        raise FileNotFoundError(
            "Merged titles JSON not found in dubbing-flask.\n"
            "Looked for:\n"
    ...
    if not src_lowerthirds.exists():
        raise FileNotFoundError(
            "LowerThirds JSON not found in dubbing-flask.\n"
            "Looked for:\n"
            f" - {src_lowerthirds}\n\n"
            "Create it first, then re-run this automation."
        )
    ...
    run_cfg = {
        ...
        # ✅ NEW (won’t break other JSX even if unused)
