...
# ✅ JSX files live here (same as reference): dubbing-flask/Javascript/
JSX_DIRNAME_AT_ROOT = "Javascript"

PIPELINE_SCRIPTS = [
    "_ImportXML.jsx",
    "_RemoveV5V6A5TrackItemsV2.jsx",
...
def main():
    # Same as reference: dubbing-flask/Python/V1/<this_file>.py => parents[2] is dubbing-flask/
    ...
    # 7) Copy JSX scripts into runner (FROM dubbing-flask/Javascript/)
    win_script_paths = []
    for fn in PIPELINE_SCRIPTS:
...
(function () {{
  ...
  // ✅ NEW: Stabilization gate + safer project opening (prevents hangs/crashes)
  ...
  // -------------------------
  // ✅ CRITICAL FIX: If project already open, DO NOT call openDocument() again
  // -------------------------
  function openProject() {{
    try {{
      if (app && app.project && app.project.rootItem && app.project.name) {{
        var desiredName = (new File(PROJECT_WIN)).name;
        if (String(app.project.name) === String(desiredName)) {{
          notify("ℹ️ Project already open (" + desiredName + ") — skipping openDocument().", "info");
          return true;
        }}
      }}
    }} catch(eSkip) {{}}
  ...
  function runOne(scriptPath, idx) {{
    ...
    // ✅ Pre-step readiness gate (prevents running while Premiere is still busy)
    if(!waitReady(30000)) {{
      notify("❌ Premiere not ready BEFORE step " + (idx+1) + " (" + f.name + "). Aborting.", "error");
      return false;
    }}
    ...
    if (ENABLE_STABILIZE_AFTER_IMPORT && String(f.name) === String(IMPORT_SCRIPT_NAME)) {{
      ...
      if (!stable) {{
        notify("❌ Stabilization timeout after XML import. Stopping pipeline to prevent crash.", "error");
        return false;
      }}

      sleep(STABILIZE_EXTRA_SLEEP_MS);

      // One more readiness gate after stabilization
      if(!waitReady(30000)) {{
    ...
    if(!waitReady(30000)) {{
      notify("❌ Premiere not ready AFTER step " + (idx+1) + " (" + f.name + "). Aborting.", "error");
      return false;
    }}
