...
(function () {
  ...
  function trySpeedToFit(trackItem, desiredStartTicks, desiredEndTicks, ticksPerFrame) {
    try {
      ...
      // Apply speed (native)
      var ok = setSpeedCompat(trackItem, speedPct);
      if (!ok) return false;
  
      // Re-anchor start after retime
      setStartTicksSafe(trackItem, desiredStartTicks);
  
      // Verify duration purely via speed (NO trimming)
      var afterDur = getTimelineDurationTicks(trackItem);
      if (!afterDur || isNaN(afterDur) || afterDur <= 0) return false;
  
      // If not close enough, fail (do NOT trim)
      if (!ticksCloseEnough(afterDur, desiredDur, 2)) return false;
    ...
    function processTrack(trackIndex, trackName, segments) {
      ...
      for (var b = 0; b < n; b++) {
        ...
        if (mvOK && ticksCloseEnough(afterStartTicks, desiredStartTicks, 2)) okMove++;
         ...
         // fit end using Selection Tool style (trim/extend duration, DO NOT change speed)
      ...
      if (n > 0 && okDurFit === 0 && deleteCount === 0) {
        fail("All end-fit updates failed on " + trackName + ". Duration-fit (trim/extend) failed.");
        return false;
      }
